...
class RobotControlApp:
    ...
    def send_command(self, cmd, is_button):
        ...
        async def send_coroutine():  # Promenjeno u async def
            if self.ws and not self.ws.closed:
                try:
                    # Koristimo await umesto yield from
                    await self.ws.send(cmd)
                    print(f"Uspešno poslato: {cmd}")
                except Exception as e:
                    print(f"Greška pri slanju: {e}")
            else:
                print("WebSocket nije spojen!")
        
        if self.ws_loop and self.ws_loop.is_running():
            # Šaljemo korutinu kroz ispravan event loop
            asyncio.run_coroutine_threadsafe(
                send_coroutine(),  # Šaljemo direktno korutinu
                self.ws_loop
            )
        ...
        while True:
            try:
                async with websockets.connect(self.ws_uri.get()) as websocket:
                    ...
                    # Dodajemo task za čekanje poruka
                    self.receive_task = asyncio.create_task(self.receive_messages())
                    
                    # Glavna petlja za održavanje konekcije
                    while True:
                        await asyncio.sleep(1)
                        # Provera da li je konekcija još aktivna
                        if websocket.closed:
                            break
    ...
    async def receive_messages(self):
        """Handlovanje dolaznih poruka sa servera"""
        try:
            async for message in self.ws:
                print(f"Primljena poruka sa servera: {message}")
        except Exception as e:
            print(f"Greška pri primanju poruka: {e}")

    def run_websocket(self):
        # Kreiranje novog event loop-a za WebSocket thread
    ...
    def send_command(self, cmd, is_button):
        ...
        def send_coroutine():
            if self.ws and not self.ws.closed:
                try:
                    # Eksplicitno šaljemo poruku kroz WebSocket
                    yield from self.ws.send(cmd)
                    print(f"Uspešno poslato: {cmd}")
                except Exception as e:
                    print(f"Greška pri slanju: {e}")
            else:
        ...
        if self.ws_loop.is_running():
            # Šaljemo komandu kroz ispravan event loop
            asyncio.run_coroutine_threadsafe(
                send_coroutine(),
                self.ws_loop
            )
