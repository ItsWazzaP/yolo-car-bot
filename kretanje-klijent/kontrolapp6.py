...
class RobotControlApp:
    def __init__(self, root):
        ...
        self.image_url = tk.StringVar(value="http://192.168.8.111:1607/capture")
    ...
    def create_controls(self):
        ...
        self.btn_up = ttk.Button(control_frame, text="⬆ Napred", command=lambda: self.send_command('napred', True))
        self.btn_down = ttk.Button(control_frame, text="⬇ Nazad", command=lambda: self.send_command('nazad', True))
        ...
        self.btn_stop = ttk.Button(control_frame, text="🛑 Stop", command=lambda: self.send_command('stop', True))

        self.btn_stop.grid(row=1, column=1)  # Centar kontrolnog panela        
    ...
    def send_combined_command(self):
        if not self.active_commands:
            ...
            self.send_command(next(iter(self.active_commands)), False)
        else:
            # Kombinacije kao napred+levo mogu biti validne
            combined = "+".join(sorted(self.active_commands))
            self.send_command(combined, False)
    ...
    def capture_image(self):
        try:
            response = requests.get(self.image_url.get(), timeout=10)
            if response.status_code == 200:
                image = Image.open(io.BytesIO(response.content))
                image = ImageOps.fit(image, (640, 480), Image.LANCZOS)
