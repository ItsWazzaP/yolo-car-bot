...
class YOLOAnnotationApp:
    ...
    def clear_annotations(self):
        ...
        messagebox.showinfo("Cleared", f"Annotations cleared for image {self.current_image_index+1}/{len(self.image_list)}")
    ...
    def load_current_image(self):
        ...
        # Update status bar
        self.status.config(text=f"Image {self.current_image_index+1}/{len(self.image_list)} | Current: {filename}")
        ...
        if filename in self.annotations:
            for ann in self.annotations[filename]:
                ...
                self.canvas.create_text(x1, y1, 
                    text=self.id_class_map.get(ann['class_id'], 'unknown'), 
